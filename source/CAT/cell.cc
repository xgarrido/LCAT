...
namespace CAT {
  ...
  void cell::set_position(const geomtools::vector_3d & p_)
  {
    _position_ = p_;
    return;
  }
  ...
  {
    if (_radius_ <= _small_radius_) return true;
    return false;
  }

  const geomtools::vector_3d & cell::get_position() const
  {
    return _position_;
  }

  double cell::get_radius() const
  {
    return _radius_;
  }
