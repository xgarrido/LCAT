...
namespace CAT{
  ...
  bool CircleRegression::fit(void){
    ...
    if( method1 ){
      ...
      for(std::vector<experimental_double>::iterator it=xi_.begin(); it != xi_.end(); ++it)
        {
          experimental_point local_point(*it, experimental_double(0.,0.), yi_[it - xi_.begin()]);
          experimental_point circle_point = c_.position(local_point);
          experimental_vector distance(local_point, circle_point);
