...
namespace CAT {
                       ...
                       bool /* a_inherit */          ) const
  {
    ...
    a_out << indent << "scenario -------------- " << std::endl;
  ...
  //!get n free families
  size_t scenario::n_free_families() const
  {
    return n_free_families_;
  }

  //!get n overlaps
  size_t scenario::n_overlaps() const
  {
    return n_overlaps_;
  }


  void scenario::calculate_n_overlaps(const std::vector<cell> & cells,
  ...
  void scenario::calculate_n_free_families(const std::vector<cell> &cells,
                                           const std::vector<calorimeter_hit> & calos)
  {
    std::vector<int> freecells(cells.size());
    fill(freecells.begin(), freecells.end(), 1);
