...
#ifndef FALAISE_CAT_SEQUENCE_H
#define FALAISE_CAT_SEQUENCE_H
...
namespace CAT {
  ...
  public:
    ...
    bool contained(const sequence & big)const;

    bool contained_same_size_and_cells(const sequence & big)const;

    bool contained_same_extreme_quadrants(const sequence & big)const;

    bool is_bridge(const sequence & first,const sequence & second)const;
    ...
    void get_chi2_change_for_changing_end_of_sequence(const experimental_point &epa, const experimental_point &epb, double *delta_chi_A, double *delta_chi_alpha);
    ...
    bool same_families(const sequence & s)const;
    ...
    bool worst_node(node& n)const;

    double phi_kink(size_t inode)const;

    // get node of largest horizontal kink
    bool largest_kink_node(node& n, double& phi)const;
