...
namespace CAT {
  ...
  bool cluster::start_ambiguity(size_t i)
  {
    ...
    std::vector<joint> joints = nodes_[i].ccc()[0].joints();
  ...
  void cluster::solve_ambiguities(std::vector< std::vector<broken_line> > * sets_of_bl_alternatives)
  {
  ...
  std::vector<broken_line> cluster::solve_ambiguities_with_ends__1_node(size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second, bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one)
  {
    ...
    if( first_ambiguous_is_after_gap ){
      ...
      if( last_ambiguous_is_last_but_one ){ // node after last ambiguous is the last:  gap - A - N |
        ...
        for( size_t joint_index = 0; joint_index <= 1; ++joint_index){
          ...
          experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index].epb();
          experimental_point pN = nodes_[ifirst].ccc()[0].joints()[joint_index].epc();
    ...
    if( first_ambiguous_is_second ){
      ...
      if( last_ambiguous_is_begore_gap ){
        ...
        for( size_t joint_index = 0; joint_index <= 1; ++joint_index){
          ...
          bl.set_ilast(ifirst);
          experimental_point p0 = nodes_[ifirst].ccc()[0].joints()[joint_index].epa();
          experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index].epb();
    ...
    experimental_point pb = nodes_[ilast+1].ccc()[0].joints()[0].epb();
    broken_line best_bl;
  ...
  std::vector<broken_line> cluster::solve_ambiguities_with_ends__2_nodes(size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second, bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one)
  {
    ...
    if( first_ambiguous_is_after_gap ){
      ...
      if( last_ambiguous_is_last_but_one ){ // node after last ambiguous is the last:  gap - A - B - N |
        ...
        for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
          for( size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B){
            ...
            experimental_point pN = nodes_[ilast].ccc()[0].joints()[joint_index_B].epc();
            bl.eps_.push_back(pA);
            bl.eps_.push_back(pB);
            bl.eps_.push_back(pN);
            bls.push_back(bl);
      ...
      for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
        experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
        experimental_point pc = nodes_[ilast+1].ccc()[0].joints()[0].epb();
        broken_line best_bl;
        best_bl.set_ifirst(ifirst);
      ...
      if( last_ambiguous_is_begore_gap ){ // 0 - A - B - gap
        ...
        for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
          for( size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B){
            ...
            bl.set_ifirst(ifirst-1);
            bl.set_ilast(ilast);
            experimental_point p0 = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epa();
            experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
            experimental_point pB = nodes_[ilast].ccc()[0].joints()[joint_index_B].epb();
            bl.eps_.push_back(p0);
            bl.eps_.push_back(pA);
  ...
  std::vector<broken_line> cluster::solve_ambiguities_with_ends__3_nodes(size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second, bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one)
  {
    ...
    if( first_ambiguous_is_after_gap ){
      ...
      if( last_ambiguous_is_last_but_one ){ // node after last ambiguous is the last:  gap - A - B - C - N
        ...
        for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
          for( size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C){
            experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
            experimental_point pC = nodes_[ilast].ccc()[0].joints()[joint_index_C].epb();
            experimental_point pN = nodes_[ilast].ccc()[0].joints()[joint_index_C].epc();
            broken_line best_bl;
            best_bl.set_ifirst(ifirst);
            best_bl.set_ilast(ilast+1);
            experimental_point pBbest;
      ...
      for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
        experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
        experimental_point pd = nodes_[ilast+1].ccc()[0].joints()[0].epb();
        broken_line best_bl;
        best_bl.set_ifirst(ifirst);
        best_bl.set_ilast(ilast);
        experimental_point pBbest;
  ...
  std::vector<broken_line> cluster::solve_ambiguities_with_ends__4_nodes(size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second, bool last_ambiguous_is_begore_gap, bool last_ambiguous_is_last_but_one){
    ...
    if( first_ambiguous_is_after_gap ){
      ...
      if( last_ambiguous_is_last_but_one ){ // node after last ambiguous is the last:  gap - A - B - C - D - N
        ...
        for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
          for( size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D){
            experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
            experimental_point pD = nodes_[ilast].ccc()[0].joints()[joint_index_D].epb();
            experimental_point pN = nodes_[ilast].ccc()[0].joints()[joint_index_D].epc();
            broken_line best_bl;
            best_bl.set_ifirst(ifirst);
            best_bl.set_ilast(ilast+1);
            experimental_point pBbest;
            ...
            for( size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B){
              for( size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C){
                broken_line bl;
                experimental_point pB = nodes_[ifirst+1].ccc()[0].joints()[joint_index_B].epb();
                experimental_point pC = nodes_[ilast-1].ccc()[0].joints()[joint_index_C].epb();
                bl.eps_.push_back(pA);
                bl.eps_.push_back(pB);
                bl.eps_.push_back(pC);
                bl.eps_.push_back(pD);
      ...
      for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
        experimental_point pA = nodes_[ifirst].ccc()[0].joints()[joint_index_A].epb();
        experimental_point pe = nodes_[ilast+1].ccc()[0].joints()[0].epb();
        broken_line best_bl;
        best_bl.set_ifirst(ifirst);
        best_bl.set_ilast(ilast);
        experimental_point pBbest;
  ...
  void cluster::solve_ambiguities_with_ends__more_than_4_nodes(broken_line ACD[2][2][2], size_t ifirst, size_t ilast, bool first_ambiguous_is_after_gap, bool first_ambiguous_is_second)
  {
  ...
  void cluster::solve_ambiguities_with_ends__more_than_4_nodes(broken_line aACD[2][2][2][2], size_t ifirst, size_t ilast)
  {
    ...
    for( size_t joint_index_AN = 0; joint_index_AN <= 1; ++joint_index_AN){
      experimental_point ante = nodes_[ifirst-1].ccc()[0].joints()[joint_index_AN].epb();
    ...
    for( size_t joint_index_AN = 0; joint_index_AN <= 1; ++joint_index_AN){
      for( size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C){
        for( size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D){
          ...
          for( size_t joint_index_A = 0; joint_index_A <= 1; ++joint_index_A){
            for( size_t joint_index_B = 0; joint_index_B <= 1; ++joint_index_B){
              broken_line bl1 = old_ACD[joint_index_AN][joint_index_A][joint_index_B];
              broken_line bl2 = aACD[joint_index_A][joint_index_B][joint_index_C][joint_index_D];
              bl1.calculate_chi2();
              bl2.calculate_chi2();
          ...
          broken_line best_bl1 = old_ACD[joint_index_AN][bestA][bestB];
          broken_line best_bl2 = aACD[bestA][bestB][joint_index_C][joint_index_D];
          broken_line best_bl = best_bl1;
  ...
  std::vector<broken_line> cluster::finish__more_than_4_nodes(broken_line ACD[2][2][2], size_t ipivot, size_t n_residuals)
  {
        ...
        for( size_t joint_index_Z = 0; joint_index_Z <= 1; ++joint_index_Z){
          ...
          for( size_t joint_index_C = 0; joint_index_C <= 1; ++joint_index_C){
            for( size_t joint_index_D = 0; joint_index_D <= 1; ++joint_index_D){
              ...
              broken_line bl1 = ACD[joint_index_AN][joint_index_C][joint_index_D];
  ...
  std::vector<broken_line> cluster::solve_ambiguities_with_ends(size_t ifirst, size_t ilast)
  {
    ...
    size_t new_ilast;
    broken_line ACD[2][2][2];
    broken_line aACD[2][2][2][2];
    bool first = true;
