...
namespace snemo {
  ...
  namespace reconstruction {
    ...
    void cat_tracker_clustering_module::set_geometry_manager(const geomtools::manager & gmgr_)
    {
      ...
      DT_THROW_IF(! std::regex_match(setup_label, std::regex("snemo::.*")),
                  std::logic_error,
                                                   ...
                                                   dpp::module_handle_dict_type & /* module_dict_ */)
    {
      ...
      const std::string algorithm_id = cat_driver::get_id();
      _driver_.reset(new snemo::reconstruction::cat_driver);
    ...
    void cat_tracker_clustering_module::_process(const snemo::datamodel::calibrated_data & calibrated_data_,
                                                 snemo::datamodel::tracker_clustering_data & clustering_data_)
    {
      DT_LOG_TRACE(get_logging_priority(), "Entering...");

      // Process the clusterizer driver :
      _driver_.get()->process(calibrated_data_.calibrated_tracker_hits(),
...
DOCD_CLASS_IMPLEMENT_LOAD_BEGIN(snemo::reconstruction::cat_tracker_clustering_module, ocd_)
{
  ...
  ocd_.set_class_library("Falaise_LCAT");
